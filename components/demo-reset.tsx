  ...
  return <>
    ...
    {open && <div className="reset-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close() }}>
      <section className="reset-dialog" role="dialog" aria-modal="true" aria-labelledby="reset-title">
        ...
        <p>This permanently removes every imported badge profile, connection, observation, analysis, and badge-linked social record. There is no undo.</p>
        <form onSubmit={reset}>
          <div className="reset-actions"><button type="button" onClick={close} disabled={loading}>Cancel</button><button type="submit" className="reset-danger" disabled={loading}>{loading ? 'Clearing…' : 'Delete badge data'}</button></div>
        </form>
