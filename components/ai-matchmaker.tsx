...
function DateView({ profiles, back }: { profiles: Record<PersonKey, Persona>; back: () => void }) {
  ...
  const startConversation = async () => {
    ...
    try {
      const response = await fetch('/api/match/conversation', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ participants: profiles, turns: 6 }), signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS) })
