...
const targetUrl =
  process.argv[2] || 'https://www.linkedin.com/in/jaimin-patel007/'
...
console.log(
  'Open LIVE_VIEW_URL and sign in to LinkedIn. This process will continue automatically.',
)
...
try {
  ...
  while (Date.now() < deadline) {
    ...
    authenticated = cookies.some(
      (cookie) => cookie.name === 'li_at' && cookie.value.length > 0,
    )
  ...
  if (!authenticated)
    throw new Error('Timed out before LinkedIn authentication completed')

  await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 30_000 })
  await page.waitForTimeout(3_000)
  const profileEvidence = await page
    .locator('main')
    .innerText()
    .catch(() => '')
  ...
  if (/\/(login|signup|authwall|checkpoint)(\/|\?|$)/.test(finalUrl)) {
    throw new Error(
      'LinkedIn redirected the authenticated context back to a login wall',
    )
  }
  if (!profileEvidence.toLowerCase().includes('jaimin patel')) {
    throw new Error(
      'The authenticated session did not expose the requested LinkedIn profile',
    )
  }
